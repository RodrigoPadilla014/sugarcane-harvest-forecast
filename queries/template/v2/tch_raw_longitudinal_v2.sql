-- tch_raw_longitudinal_v2.sql
--
-- Candidate raw longitudinal v2 template.
--
-- Full v1-style raw join surface, with corrected crop-window logic:
--   - target rows come from productividad
--   - STAC rows join by lote + estimated crop window, not by same cod_cg_zafra
--   - fecha_inicio_estimada = cierre_date - prod_edad * 30.44 days
--   - cierre_ciclo = productividad.cierre_date
--   - edad_de_cultivo = fecha_stac - fecha_inicio_estimada
--   - anomalous TCH targets are dropped with NOT (tch < 20 OR tch > 150)
--   - phenology flags are kept as raw QC columns
--
-- To recreate manually:
-- CREATE MATERIALIZED VIEW public.tch_raw_longitudinal_v2 AS
...
WITH base AS (
         ...
         SELECT cycle_stats.*,
            ...
            CASE
                WHEN cycle_stats.cycle_obs_count >= 7
                 AND (cycle_stats.ndvi_max - cycle_stats.ndvi_min) >= 0.35::double precision
                 AND cycle_stats.edad_en_pico_ndvi BETWEEN 45 AND 360 THEN true
                ELSE false
