-- tch_raw_longitudinal_v3.sql
--
-- Candidate raw longitudinal v3 template.
--
-- Same window logic and join surface as v2 (cierre - prod_edad*30.44 days
-- to cierre), same TCH bounds [20, 150]. The change vs v2 is the phenology
-- gate: instead of amplitude + absolute peak-age, v3 uses a shape-based gate
-- ("E3") that validates the canonical sugar-cane sinusoid directly.
--
--   v3 phenology_valid =
--       cycle_obs_count            >= 7
--   AND ndvi_first_60d              < 0.35     -- low NDVI right after cut
--   AND ndvi_max                    > 0.60     -- canopy actually closes
--   AND ndvi_amp                    > 0.40     -- non-flat curve
--   AND rel_peak_pos                BETWEEN 0.25 AND 0.95  -- peak mid-late
--
-- ciclo_valido_basico is kept identical to v2:
--   cycle_obs_count >= 7 AND max_edad_de_cultivo >= 150.
--
-- Diagnostic columns added to the output:
--   ndvi_first_60d, ndvi_last_30d, rel_peak_pos.
--
-- See queries/diagnostics/v2_experiments*.sql for the audit that motivated
-- this gate: B+E3 keeps 29,052 lot-zafras with clean canonical phenology
-- (mean first_60d=0.251, peak=0.811, amp=0.814) without TCH bias.
--
-- To recreate manually:
-- CREATE MATERIALIZED VIEW public.tch_raw_longitudinal_v3 AS
...
WITH base AS (
         SELECT p.lote AS cod_cg,
           ...
           FROM productividad p
             JOIN stac_indices s ON s.lote = p.lote
              AND s.fecha::date >= (
                    to_date(p.cierre, 'DD/MM/YYYY'::text)
         ...
         SELECT b.*,
            ...
            (
                ...
                AND b.fecha_stac <= b.cierre_date
            ) AS dentro_ventana_cultivo
           FROM base b
        ), with_age AS (
         SELECT w.*,
            CASE
         ...
         SELECT with_age.cod_cg_zafra,
            ...
            -- window length in days, taken from the productividad row (constant
            -- per cod_cg_zafra; max() is just to pick the value out of the group).
