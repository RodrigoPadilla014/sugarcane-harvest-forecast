-- tch_features_v8_dynamic_radar.sql
--
...
--     radar/climate source zafra labels.
--
-- Source tables:
...
open_latest_observation AS (
    ...
    LEFT JOIN radar_lote_lookup rl
      ON rl.cod_cg = o.cod_cg
    LEFT JOIN public.radar s
      ON s.lote = rl.radar_lote_matched
     AND s.fecha::date >= o.fecha_inicio_ciclo
     AND NULLIF(s.vv_promedio::text, 'NaN')::double precision IS NOT NULL
     AND NULLIF(s.vh_promedio::text, 'NaN')::double precision IS NOT NULL
...
radar_observations AS (
    SELECT
        b.cod_cg_zafra,
        r.fecha::date AS fecha_obs,
        (r.fecha::date - b.fecha_inicio_ciclo)::double precision AS age_days,
        b.as_of_age_days,
        lower(r.orbit_pass) AS orbit_name,
        CASE
            WHEN (r.fecha::date - b.fecha_inicio_ciclo) BETWEEN 0 AND 90 THEN 'age_000_090'
            WHEN (r.fecha::date - b.fecha_inicio_ciclo) BETWEEN 91 AND 180 THEN 'age_091_180'
            WHEN (r.fecha::date - b.fecha_inicio_ciclo) BETWEEN 181 AND b.as_of_age_days THEN 'age_181_asof'
            ELSE 'outside'
        END AS age_window,
        (r.fecha::date - b.fecha_inicio_ciclo) >= greatest(0.0, b.as_of_age_days - 30.0) AS in_last_30d,
        (r.fecha::date - b.fecha_inicio_ciclo) >= greatest(0.0, b.as_of_age_days - 60.0) AS in_last_60d,
    ...
    JOIN radar_lote_lookup rl
      ON rl.cod_cg = b.cod_cg
    JOIN public.radar r
      ON r.lote = rl.radar_lote_matched
     AND r.fecha::date BETWEEN b.fecha_inicio_ciclo AND b.as_of_date
...
radar_dates_prepared AS (
    ...
    FROM (
        SELECT DISTINCT cod_cg_zafra, fecha_obs, age_days
...
radar_coverage AS (
    SELECT
        cod_cg_zafra,
        count(*)::double precision AS radar_obs_count_0_asof,
        count(*) FILTER (WHERE age_days BETWEEN 0 AND 180)::double precision AS radar_obs_count_0_180,
        count(*) FILTER (WHERE age_days BETWEEN 181 AND 340)::double precision AS radar_obs_count_181_asof,
        min(age_days) FILTER (WHERE age_days BETWEEN 0 AND 180) AS radar_first_obs_age_0_180,
        max(age_days) FILTER (WHERE age_days BETWEEN 0 AND 180) AS radar_last_obs_age_0_180,
        max(age_days - previous_age_days) FILTER (
            WHERE age_days BETWEEN 0 AND 180
        ) AS radar_max_internal_gap_days_0_180,
        greatest(
            min(age_days) FILTER (WHERE age_days BETWEEN 0 AND 180),
            COALESCE(max(age_days - previous_age_days) FILTER (
                WHERE age_days BETWEEN 0 AND 180
            ), 0),
            180.0 - max(age_days) FILTER (WHERE age_days BETWEEN 0 AND 180)
        ) AS radar_boundary_max_gap_days_0_180
    FROM radar_dates_prepared
    GROUP BY cod_cg_zafra
),
radar_orbit_coverage AS (
    SELECT
        cod_cg_zafra,
        count(DISTINCT fecha_obs) FILTER (
            WHERE orbit_name = 'ascending' AND age_days BETWEEN 0 AND 180
        )::double precision AS radar_asc_obs_count_0_180,
        count(DISTINCT fecha_obs) FILTER (
            WHERE orbit_name = 'descending' AND age_days BETWEEN 0 AND 180
        )::double precision AS radar_desc_obs_count_0_180,
        bool_or(orbit_name = 'ascending') AS radar_has_ascending,
        bool_or(orbit_name = 'descending') AS radar_has_descending
    FROM radar_observations
    GROUP BY cod_cg_zafra
),
radar_metric_long AS (
    SELECT
        o.cod_cg_zafra,
        o.fecha_obs,
        o.age_days,
...
radar_metric_features AS (
    SELECT
        cod_cg_zafra,
        orbit_name,
        metric_name,
        avg(metric_value) AS mean_0_asof,
        avg(metric_value) FILTER (WHERE age_days BETWEEN 0 AND 180) AS mean_0_180,
        avg(metric_value) FILTER (WHERE age_window = 'age_181_asof') AS mean_181_asof,
        avg(metric_value) FILTER (WHERE in_last_30d) AS mean_last30,
        avg(metric_value) FILTER (WHERE in_last_60d) AS mean_last60,
        max(metric_value) AS peak_0_asof,
        max(metric_value) FILTER (WHERE age_days BETWEEN 0 AND 180) AS peak_0_180,
        min(metric_value) FILTER (WHERE age_days BETWEEN 0 AND 180) AS min_0_180,
        max(metric_value) - min(metric_value) AS amplitude_0_asof,
        max(metric_value) FILTER (WHERE age_days BETWEEN 0 AND 180)
            - min(metric_value) FILTER (WHERE age_days BETWEEN 0 AND 180) AS amplitude_0_180,
        regr_slope(metric_value, age_days) AS slope_0_asof,
        regr_slope(metric_value, age_days) FILTER (WHERE age_days BETWEEN 0 AND 180) AS slope_0_180,
        regr_slope(metric_value, age_days) FILTER (WHERE age_window = 'age_181_asof') AS slope_181_asof,
        (array_agg(metric_value ORDER BY age_days DESC, fecha_obs DESC))[1] AS last_value_before_asof,
        percentile_cont(0.95) WITHIN GROUP (ORDER BY metric_value) FILTER (
            WHERE age_days BETWEEN 0 AND 180
        ) AS p95_0_180
    FROM radar_metric_long
...
quality_flags AS (
    SELECT
        b.cod_cg_zafra,
        COALESCE(r.radar_obs_count_0_180, 0) >= 7 AS asof_radar_obs_ok,
        COALESCE(r.radar_boundary_max_gap_days_0_180, 9999) <= 60 AS asof_radar_gap_ok,
        ...
        (
            r.radar_obs_count_0_180 >= 7
            AND COALESCE(r.radar_boundary_max_gap_days_0_180, 9999) <= 60
            AND COALESCE(c.climate_pentad_count_0_180, 0) >= 30
            AND b.cycle_duration_days BETWEEN 180 AND (14.0 * 30.44)
        ) AS asof_valid
    FROM eligible_cycles b
    LEFT JOIN radar_coverage r USING (cod_cg_zafra)
...
SELECT
    ...
    rl.radar_lote_matched,
    rl.radar_lote_match_method,
    ...
    r.radar_obs_count_0_180,
    r.radar_obs_count_0_asof,
    r.radar_obs_count_181_asof,
    r.radar_first_obs_age_0_180,
    r.radar_last_obs_age_0_180,
    r.radar_max_internal_gap_days_0_180,
    r.radar_boundary_max_gap_days_0_180,
    ro.radar_asc_obs_count_0_180,
    ro.radar_desc_obs_count_0_180,
    ro.radar_has_ascending,
    ro.radar_has_descending,

    asc_rvi.mean_0_180 AS radar_asc_rvi_mean_0_180,
    asc_rvi.mean_0_asof AS radar_asc_rvi_mean_0_asof,
    asc_rvi.mean_181_asof AS radar_asc_rvi_mean_181_asof,
    asc_rvi.mean_last30 AS radar_asc_rvi_mean_last30,
    asc_rvi.mean_last60 AS radar_asc_rvi_mean_last60,
    asc_rvi.peak_0_180 AS radar_asc_rvi_peak_0_180,
    asc_rvi.peak_0_asof AS radar_asc_rvi_peak_0_asof,
    asc_rvi.min_0_180 AS radar_asc_rvi_min_0_180,
    asc_rvi.amplitude_0_180 AS radar_asc_rvi_amplitude_0_180,
    asc_rvi.amplitude_0_asof AS radar_asc_rvi_amplitude_0_asof,
    asc_rvi.slope_0_180 AS radar_asc_rvi_slope_0_180,
    asc_rvi.slope_0_asof AS radar_asc_rvi_slope_0_asof,
    asc_rvi.slope_181_asof AS radar_asc_rvi_slope_181_asof,
    asc_rvi.last_value_before_asof AS radar_asc_rvi_last_value_before_asof,
    asc_rvi.p95_0_180 AS radar_asc_rvi_p95_0_180,

    desc_rvi.mean_0_180 AS radar_desc_rvi_mean_0_180,
    desc_rvi.mean_0_asof AS radar_desc_rvi_mean_0_asof,
    desc_rvi.mean_181_asof AS radar_desc_rvi_mean_181_asof,
    desc_rvi.mean_last30 AS radar_desc_rvi_mean_last30,
    desc_rvi.mean_last60 AS radar_desc_rvi_mean_last60,
    desc_rvi.peak_0_180 AS radar_desc_rvi_peak_0_180,
    desc_rvi.peak_0_asof AS radar_desc_rvi_peak_0_asof,
    desc_rvi.min_0_180 AS radar_desc_rvi_min_0_180,
    desc_rvi.amplitude_0_180 AS radar_desc_rvi_amplitude_0_180,
    desc_rvi.amplitude_0_asof AS radar_desc_rvi_amplitude_0_asof,
    desc_rvi.slope_0_180 AS radar_desc_rvi_slope_0_180,
    desc_rvi.slope_0_asof AS radar_desc_rvi_slope_0_asof,
    desc_rvi.slope_181_asof AS radar_desc_rvi_slope_181_asof,
    desc_rvi.last_value_before_asof AS radar_desc_rvi_last_value_before_asof,
    desc_rvi.p95_0_180 AS radar_desc_rvi_p95_0_180,

    asc_vv.mean_0_180 AS radar_asc_vv_mean_0_180,
    asc_vv.mean_0_asof AS radar_asc_vv_mean_0_asof,
    asc_vv.mean_181_asof AS radar_asc_vv_mean_181_asof,
    asc_vv.mean_last30 AS radar_asc_vv_mean_last30,
    asc_vv.mean_last60 AS radar_asc_vv_mean_last60,
    asc_vv.peak_0_180 AS radar_asc_vv_peak_0_180,
    asc_vv.peak_0_asof AS radar_asc_vv_peak_0_asof,
    asc_vv.min_0_180 AS radar_asc_vv_min_0_180,
    asc_vv.amplitude_0_180 AS radar_asc_vv_amplitude_0_180,
    asc_vv.amplitude_0_asof AS radar_asc_vv_amplitude_0_asof,
    asc_vv.slope_0_180 AS radar_asc_vv_slope_0_180,
    asc_vv.slope_0_asof AS radar_asc_vv_slope_0_asof,
    asc_vv.slope_181_asof AS radar_asc_vv_slope_181_asof,
    asc_vv.last_value_before_asof AS radar_asc_vv_last_value_before_asof,
    asc_vv.p95_0_180 AS radar_asc_vv_p95_0_180,

    desc_vv.mean_0_180 AS radar_desc_vv_mean_0_180,
    desc_vv.mean_0_asof AS radar_desc_vv_mean_0_asof,
    desc_vv.mean_181_asof AS radar_desc_vv_mean_181_asof,
    desc_vv.mean_last30 AS radar_desc_vv_mean_last30,
    desc_vv.mean_last60 AS radar_desc_vv_mean_last60,
    desc_vv.peak_0_180 AS radar_desc_vv_peak_0_180,
    desc_vv.peak_0_asof AS radar_desc_vv_peak_0_asof,
    desc_vv.min_0_180 AS radar_desc_vv_min_0_180,
    desc_vv.amplitude_0_180 AS radar_desc_vv_amplitude_0_180,
    desc_vv.amplitude_0_asof AS radar_desc_vv_amplitude_0_asof,
    desc_vv.slope_0_180 AS radar_desc_vv_slope_0_180,
    desc_vv.slope_0_asof AS radar_desc_vv_slope_0_asof,
    desc_vv.slope_181_asof AS radar_desc_vv_slope_181_asof,
    desc_vv.last_value_before_asof AS radar_desc_vv_last_value_before_asof,
    desc_vv.p95_0_180 AS radar_desc_vv_p95_0_180,

    asc_vh.mean_0_180 AS radar_asc_vh_mean_0_180,
    asc_vh.mean_0_asof AS radar_asc_vh_mean_0_asof,
    asc_vh.mean_181_asof AS radar_asc_vh_mean_181_asof,
    asc_vh.mean_last30 AS radar_asc_vh_mean_last30,
    asc_vh.mean_last60 AS radar_asc_vh_mean_last60,
    asc_vh.peak_0_180 AS radar_asc_vh_peak_0_180,
    asc_vh.peak_0_asof AS radar_asc_vh_peak_0_asof,
    asc_vh.min_0_180 AS radar_asc_vh_min_0_180,
    asc_vh.amplitude_0_180 AS radar_asc_vh_amplitude_0_180,
    asc_vh.amplitude_0_asof AS radar_asc_vh_amplitude_0_asof,
    asc_vh.slope_0_180 AS radar_asc_vh_slope_0_180,
    asc_vh.slope_0_asof AS radar_asc_vh_slope_0_asof,
    asc_vh.slope_181_asof AS radar_asc_vh_slope_181_asof,
    asc_vh.last_value_before_asof AS radar_asc_vh_last_value_before_asof,
    asc_vh.p95_0_180 AS radar_asc_vh_p95_0_180,

    desc_vh.mean_0_180 AS radar_desc_vh_mean_0_180,
    desc_vh.mean_0_asof AS radar_desc_vh_mean_0_asof,
    desc_vh.mean_181_asof AS radar_desc_vh_mean_181_asof,
    desc_vh.mean_last30 AS radar_desc_vh_mean_last30,
    desc_vh.mean_last60 AS radar_desc_vh_mean_last60,
    desc_vh.peak_0_180 AS radar_desc_vh_peak_0_180,
    desc_vh.peak_0_asof AS radar_desc_vh_peak_0_asof,
    desc_vh.min_0_180 AS radar_desc_vh_min_0_180,
    desc_vh.amplitude_0_180 AS radar_desc_vh_amplitude_0_180,
    desc_vh.amplitude_0_asof AS radar_desc_vh_amplitude_0_asof,
    desc_vh.slope_0_180 AS radar_desc_vh_slope_0_180,
    desc_vh.slope_0_asof AS radar_desc_vh_slope_0_asof,
    desc_vh.slope_181_asof AS radar_desc_vh_slope_181_asof,
    desc_vh.last_value_before_asof AS radar_desc_vh_last_value_before_asof,
    desc_vh.p95_0_180 AS radar_desc_vh_p95_0_180,

    asc_ratio.mean_0_180 AS radar_asc_ratio_mean_0_180,
    asc_ratio.mean_0_asof AS radar_asc_ratio_mean_0_asof,
    asc_ratio.mean_181_asof AS radar_asc_ratio_mean_181_asof,
    asc_ratio.mean_last30 AS radar_asc_ratio_mean_last30,
    asc_ratio.mean_last60 AS radar_asc_ratio_mean_last60,
    asc_ratio.peak_0_180 AS radar_asc_ratio_peak_0_180,
    asc_ratio.peak_0_asof AS radar_asc_ratio_peak_0_asof,
    asc_ratio.min_0_180 AS radar_asc_ratio_min_0_180,
    asc_ratio.amplitude_0_180 AS radar_asc_ratio_amplitude_0_180,
    asc_ratio.amplitude_0_asof AS radar_asc_ratio_amplitude_0_asof,
    asc_ratio.slope_0_180 AS radar_asc_ratio_slope_0_180,
    asc_ratio.slope_0_asof AS radar_asc_ratio_slope_0_asof,
    asc_ratio.slope_181_asof AS radar_asc_ratio_slope_181_asof,
    asc_ratio.last_value_before_asof AS radar_asc_ratio_last_value_before_asof,
    asc_ratio.p95_0_180 AS radar_asc_ratio_p95_0_180,

    desc_ratio.mean_0_180 AS radar_desc_ratio_mean_0_180,
    desc_ratio.mean_0_asof AS radar_desc_ratio_mean_0_asof,
    desc_ratio.mean_181_asof AS radar_desc_ratio_mean_181_asof,
    desc_ratio.mean_last30 AS radar_desc_ratio_mean_last30,
    desc_ratio.mean_last60 AS radar_desc_ratio_mean_last60,
    desc_ratio.peak_0_180 AS radar_desc_ratio_peak_0_180,
    desc_ratio.peak_0_asof AS radar_desc_ratio_peak_0_asof,
    desc_ratio.min_0_180 AS radar_desc_ratio_min_0_180,
    desc_ratio.amplitude_0_180 AS radar_desc_ratio_amplitude_0_180,
    desc_ratio.amplitude_0_asof AS radar_desc_ratio_amplitude_0_asof,
    desc_ratio.slope_0_180 AS radar_desc_ratio_slope_0_180,
    desc_ratio.slope_0_asof AS radar_desc_ratio_slope_0_asof,
    desc_ratio.slope_181_asof AS radar_desc_ratio_slope_181_asof,
    desc_ratio.last_value_before_asof AS radar_desc_ratio_last_value_before_asof,
    desc_ratio.p95_0_180 AS radar_desc_ratio_p95_0_180,
...
LEFT JOIN climate_lote_lookup cl
  ON cl.cod_cg = b.cod_cg
LEFT JOIN radar_coverage r
  ON r.cod_cg_zafra = b.cod_cg_zafra
LEFT JOIN radar_orbit_coverage ro
  ON ro.cod_cg_zafra = b.cod_cg_zafra
LEFT JOIN radar_metric_features asc_rvi
  ON asc_rvi.cod_cg_zafra = b.cod_cg_zafra
 AND asc_rvi.orbit_name = 'ascending'
 AND asc_rvi.metric_name = 'rvi'
LEFT JOIN radar_metric_features desc_rvi
  ON desc_rvi.cod_cg_zafra = b.cod_cg_zafra
 AND desc_rvi.orbit_name = 'descending'
 AND desc_rvi.metric_name = 'rvi'
LEFT JOIN radar_metric_features asc_vv
  ON asc_vv.cod_cg_zafra = b.cod_cg_zafra
 AND asc_vv.orbit_name = 'ascending'
 AND asc_vv.metric_name = 'vv'
LEFT JOIN radar_metric_features desc_vv
  ON desc_vv.cod_cg_zafra = b.cod_cg_zafra
 AND desc_vv.orbit_name = 'descending'
 AND desc_vv.metric_name = 'vv'
LEFT JOIN radar_metric_features asc_vh
  ON asc_vh.cod_cg_zafra = b.cod_cg_zafra
 AND asc_vh.orbit_name = 'ascending'
 AND asc_vh.metric_name = 'vh'
LEFT JOIN radar_metric_features desc_vh
  ON desc_vh.cod_cg_zafra = b.cod_cg_zafra
 AND desc_vh.orbit_name = 'descending'
 AND desc_vh.metric_name = 'vh'
LEFT JOIN radar_metric_features asc_ratio
  ON asc_ratio.cod_cg_zafra = b.cod_cg_zafra
 AND asc_ratio.orbit_name = 'ascending'
 AND asc_ratio.metric_name = 'ratio'
LEFT JOIN radar_metric_features desc_ratio
  ON desc_ratio.cod_cg_zafra = b.cod_cg_zafra
 AND desc_ratio.orbit_name = 'descending'
 AND desc_ratio.metric_name = 'ratio'
LEFT JOIN climate_core c
  ON c.cod_cg_zafra = b.cod_cg_zafra
LEFT JOIN enso_core e
  ON e.cod_cg_zafra = b.cod_cg_zafra
LEFT JOIN quality_flags q
  ON q.cod_cg_zafra = b.cod_cg_zafra;
